...
with leftcolumn:
    if st.button('Show Instagram Sentiment Analysis'):
        insta_fig = analyze_sentiments()
        if insta_fig:  # Check if the figure is generated
            st.plotly_chart(insta_fig, use_container_width=True)

with middlecolumn:
    if st.button('Show Facebook Sentiment Analysis'):
        facebook_fig = analyze_facebook_sentiments()
        if facebook_fig:  # Check if the figure is generated
            st.plotly_chart(facebook_fig, use_container_width=True)

with rightcolumn:
    if st.button('Show Twitter Sentiment Analysis'):
        twit_fig = analyze_twitter_sentiments()
        if twit_fig:  # Check if the figure is generated
